...
export default {
  ...
  methods: {
    addSelected() {
      ...
      if (this.selected) {
        this.$store.dispatch("ADD_SELECTED_DEVICES", this.item);
      } else {
        this.$store.dispatch("REMOVE_SELECTED_DEVICES", this.item);
      }
    },
  },
  mounted() {
    const index = this.$store.getters["getSelectedDevices"].findIndex(
